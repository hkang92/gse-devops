---
title: Configure Tekton Pipelines
description: Guide for configuring the theme’s various settings
---

<PageDescription>

This [README](https://github.ibm.com/CASE-DevOps-GSE/DevOps/blob/master/docs/bluecompute-ce-tekton-pipelines/web/README.md) is a follow on to deploying `bluecompute-ce` on an OpenShift cluster and will focus on three activities:

1. Configure the BlueCompute Web app deployment to pull images from the IBM Container Registry.
1. Configure and manually validate a Tekton CICD pipeline.
1. Configure a Tekton Webhook to trigger the pipeline upon a push.

</PageDescription>

![BlueCompute Devops Tekton](../gallery/BlueCompute-DevOps-Tekton.png)

<AnchorLinks>
  <AnchorLink>Requirements</AnchorLink>
  <AnchorLink>Install Tekton Pipeline, Dashboard and Webhook extension</AnchorLink>
  <AnchorLink>Configure Tekton Github Webhook</AnchorLink>
  <AnchorLink>Validate webhook by pushing in a change to the web application</AnchorLink>
  <AnchorLink>Uninstall Tekton components</AnchorLink>
</AnchorLinks>
...
- Deploy `bluecompute-ce` reference application [instructions](https://github.ibm.com/CASE-DevOps-GSE/DevOps/blob/master/docs/bluecompute-ce/README.md).
...
```
  ...
  oc get route
```
![Tekton Dashboard](../gallery/tekton-dashboard.png)
...
![Tekton Webhook Menu Selection](../gallery/tekton-webhook-menu-selection.png)
...
```
  - Pipeline
  ![Tekton Pipeline](../gallery/tekton-pipeline-web.png)
  - PipelineResources
  ![Tekton PipelineResources](../gallery/tekton-pipelineresources-web.png)
  - Tasks
  ![Tekton Tasks](../gallery/tekton-tasks-web.png)
...
*13*. Manually run the Tekton Pipeline.
  ...
  - Description of the pod status during the execution of a Tekton pipeline.  
      ...
      * Once the deployment is complete, the existing `web` application pod is terminated and a new `web` pod is started referencing the new image that was created and pushed to the IBM Container Registry.
      ![Manual run of Tekton pipeline](../gallery/tekton-pipeline-pod-status-web.png)
...
![Tekton Webhook Access Token](../gallery/tekton-access-token.png)

*4*. Fill in the fields similar to below but use your own Github repository and Docker Registry.  Click Create.
![Tekton Webhook Configuration](../gallery/tekton-webhook-config.png)

*5*. A Tekton Webhook has been configured.
![Tekton Webhook](../gallery/tekton-webhook.png)
...
*2*. Make a change to the web application homepage and commit the change.
  ...
  `kubectl get pods -w -n bluecompute`
   ![Webhook Triggered Tekton Pipeline](../gallery/webhook-tekton-pipeline-pod-status.png)
  - Alternatively, one can monitor the webhook triggered pipelinerun from the Tekton Dashboard as well.
      - PipelineRun View:
      ![Tekton Dashboard View](../gallery/tekton-dashboard-pipelinerun.png)
      - TaskRun View:
      ![Tekton Dashboard View](../gallery/tekton-dashboard-taskruns.png)
